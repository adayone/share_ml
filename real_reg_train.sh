...
rm -rf data/*.cache
rm model/zz500_model_reg.txt
rm model/mode.vw

/usr/local/bin/vw -d data/vw_train \
-c \
-f model/zz500_model_reg.vw \
--ngram 5 \
--passes 1000 \
--holdout_off \
--readable_model model/zz500_model_reg.txt 


cat data/vw_test | awk -F'|' '{print $1}'  > data/gold
/usr/local/bin/vw -d data/vw_test -t -i model/zz500_model_reg.vw -p data/pred_test
...
/usr/local/bin/vw -d data/vw_pred -t -i model/zz500_model_reg.vw -p data/pred_day
paste data/id data/pred_day > result/zz500_pred_reg
