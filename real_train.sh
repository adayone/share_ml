...
rm -rf data/*.cache
rm model/sz50_model.txt
rm model/mode.vw

/usr/local/bin/vw -d data/vw_train \
-c \
-f model/sz50_model.vw \
--ngram 5 \
--passes 1000 \
--holdout_off \
--readable_model model/sz50_model.txt 


cat data/vw_test | awk -F'|' '{print $1}'  > data/gold
/usr/local/bin/vw -d data/vw_test -t -i model/sz50_model.vw -p data/pred_test
cat data/test.csv |  awk -F',' '{print $1}' > data/id
paste data/id data/gold data/pred_test > data/pred_test_cv
/usr/local/bin/perf -roc -files data/gold data/pred_test > model/sz50_auc 
cat model/sz50_auc


cat data/predict.csv | tr -d '"' | sed 's/,/ | /g' > data/vw_pred	
cat data/vw_pred |  awk -F'|' '{print $1}' > data/id
/usr/local/bin/vw -d data/vw_pred -t -i model/sz50_model.vw -p data/pred_day
paste data/id data/pred_day > result/sz50_pred
