...
rm -rf data/*.cache
rm model/hs300_model.txt
rm model/mode.vw

/usr/local/bin/vw -d data/vw_train \
-c \
-f model/hs300_model.vw \
--ngram 5 \
--passes 1000 \
--holdout_off \
--readable_model model/hs300_model.txt 


cat data/vw_test | awk -F'|' '{print $1}'  > data/gold
/usr/local/bin/vw -d data/vw_test -t -i model/hs300_model.vw -p data/pred_test
...
/usr/local/bin/vw -d data/vw_pred -t -i model/hs300_model.vw -p data/pred_day
paste data/id data/pred_day > result/hs300_pred
